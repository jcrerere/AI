    ...
    <div className="flex flex-col h-full relative">
      ...
      <div className="mb-4 pb-2 border-b border-white/10">
        ...
        <div className="flex justify-between items-end mt-1">
          ...
          <span className="text-xs font-mono text-cyan-400 border border-cyan-900 bg-cyan-950/30 px-2 rounded">
            LV.{faction.level}
          </span>
        </div>
      </div>
            ...
            <CyberPanel title="财务报表" noPadding className="p-4">
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-xl font-bold font-mono text-yellow-400">
                  {faction.economy.treasury.toLocaleString()} 灵能币
                </span>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-900/50 p-3 border border-slate-800 rounded">
                  ...
                  <div className="text-sm font-bold text-green-400">
                    + {faction.economy.monthlyIncome.toLocaleString()} 灵能币
                  </div>
                </div>
                <div className="bg-slate-900/50 p-3 border border-slate-800 rounded">
                  <div className="flex items-center gap-1 text-[10px] text-slate-500 mb-1">
                  ...
                  <div className="text-sm font-bold text-red-400">
                    - {faction.economy.monthlyUpkeep.toLocaleString()} 灵能币
                  </div>
                </div>
              </div>
            </CyberPanel>
              ...
              <div className="space-y-2">
                {faction.logs.map(log => (
                  <div key={log.id} className="border-l-2 border-slate-700 pl-3 py-1 relative">
                    ...
                    <div
                      className={`text-xs ${log.type === 'alert' ? 'text-red-400' : log.type === 'success' ? 'text-green-400' : 'text-slate-300'}`}
                    >
                      {log.content}
                    </div>
                  </div>
        ...
        {activeTab === 'industry' && (
          <div className="space-y-3">
            {faction.industries.map(ind => (
              <div
                key={ind.id}
                className="bg-slate-900/40 border border-slate-700 p-3 flex justify-between items-start"
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-bold text-slate-200">{ind.name}</span>
                    <span
                      className={`text-[9px] px-1 rounded border ${ind.status === 'active' ? 'border-green-800 text-green-500' : 'border-yellow-800 text-yellow-500'}`}
                    >
                      {ind.status.toUpperCase()}
                    </span>
                  </div>
                  <div className="text-[10px] text-slate-500">
                    {ind.type} // LV.{ind.level}
                  </div>
                </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-900/50 p-3 border border-cyan-900/30">
              ...
              <div className="flex gap-2 mb-2">
                <input
                  type="text"
                  placeholder="任务标题"
                  className="flex-1 bg-black border border-slate-700 px-2 py-1 text-xs text-white"
                />
                <input
                  type="number"
                  placeholder="酬金"
                  className="w-20 bg-black border border-slate-700 px-2 py-1 text-xs text-white"
                />
              </div>
              <div className="flex gap-2">
                ...
                <button className="flex-1 bg-cyan-900/50 text-cyan-400 text-xs font-bold border border-cyan-800 hover:bg-cyan-800">
                  发布
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2">
              {faction.postedTasks.map(task => (
                <div
                  key={task.id}
                  className="bg-black/30 border border-slate-800 p-3 hover:border-slate-600 transition-colors"
                >
                  ...
                  <div className="flex justify-between items-center text-[10px]">
                    <span
                      className={`px-2 py-0.5 border rounded ${task.status === 'open' ? 'text-green-500 border-green-900' : 'text-slate-500 border-slate-800'}`}
                    >
                      {task.status === 'open' ? 'OPEN' : 'ASSIGNED'}
                    </span>
                    <span className="text-slate-600">
                      {task.targetAudience === 'internal' ? '内部任务' : '公开悬赏'}
                    </span>
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {faction.groups.map(grp => (
                  <div
                    key={grp.id}
                    className="bg-slate-900/40 border border-slate-700 p-2 cursor-pointer hover:border-cyan-500/50"
                  >
                  ...
                  >
                    ...
                    {selectedMember?.id === member.id && (
                      <div className="mt-3 pt-2 border-t border-slate-700 flex gap-2 animate-in fade-in">
                        <button className="flex-1 bg-cyan-900/30 border border-cyan-800 text-[10px] text-cyan-300 py-1 hover:bg-cyan-800">
                          指派任务
                        </button>
                        <button className="flex-1 bg-slate-800 border border-slate-700 text-[10px] text-slate-300 py-1 hover:bg-slate-700">
                          调整职位
                        </button>
                      </div>
                    )}
              ...
              return (
                <div key={i} className="bg-slate-900/40 border border-slate-700 p-3 group">
                  ...
                  <div className="flex gap-2 opacity-50 group-hover:opacity-100 transition-opacity">
                    <button className="flex-1 border border-slate-600 text-[9px] text-slate-300 py-1 hover:bg-slate-700">
                      委托
                    </button>
                    <button className="flex-1 border border-slate-600 text-[9px] text-slate-300 py-1 hover:bg-slate-700">
                      合作
                    </button>
                    <button className="flex-1 border border-red-900/50 text-[9px] text-red-400 py-1 hover:bg-red-900/30">
                      掠夺
                    </button>
                  </div>
                </div>
        ...
        <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-h-full bg-slate-900 border border-slate-700 flex flex-col shadow-2xl">
            <div className="p-3 border-b border-slate-700 flex justify-between items-center">
              ...
              <button onClick={() => setShowRecruitModal(false)} className="text-slate-500 hover:text-white">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2">
              ...
              {MOCK_NPCS.map(npc => (
                <div
                  key={npc.id}
                  className="flex items-center justify-between p-2 hover:bg-slate-800 cursor-pointer border border-transparent hover:border-cyan-500/30"
                >
                  <div className="flex items-center gap-2">
                    <PersistentImage src={npc.avatarUrl} alt={npc.name} className="w-6 h-6 rounded-full" />
              ...
              {[1, 2, 3].map(i => (
                ...
                >
                  ...
                  <button className="text-[9px] border border-yellow-800 text-yellow-500 px-2 py-0.5">
                    雇佣 (500 灵能币)
                  </button>
                </div>
              ))}
